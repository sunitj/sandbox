...
dataDir="/Users/sunitj/Github/sandbox/DESeq2/data"
setwd(workDir)
...
colData(ddsHTSeq)$condition<-factor(colData(ddsHTSeq)$condition, levels=c("Night","Day"))
```
...
qplot(PC1, PC2, color=name, shape=condition, data=data) +
...
mds=data.frame(mds)
qplot(X1,X2,color=name,shape=condition,data=mds)
```
...
Here are the genes with the highest counts in each sample:

##### Day1

```{r count_day1 }
countData[which.max(countData$Day_1),]
```

##### Day2

```{r count_day2}
countData[which.max(countData$Day_2),]
```

##### Day3

```{r count_day3}
countData[which.max(countData$Day_3),]
```

##### Night4

```{r count_night4}
countData[which.max(countData$Night_4),]
```

##### Night5

```{r count_night5}
countData[which.max(countData$Night_5),]
```

##### Night6

```{r count_night6}
countData[which.max(countData$Night_6),]
```
...
library("tidyr")
library("dplyr")
day.counts=countData[, c("Day_1","Day_2","Day_3")]
day.counts$Name=rownames(day.counts)
day.counts.ordered = day.counts %>% 
  gather(Days, Counts, -Name) %>%
  group_by(Name) %>%
  summarise(Totals=sum(Counts), mean=mean(Counts), se=sd(Counts)/sqrt(length(Counts))) %>%
  arrange(desc(mean))
day.counts.ordered.top100=day.counts.ordered[1:500 ,]
day.limits=aes(ymax=mean+se, ymin=mean-se)
day.counts.ordered.top100$Rank = seq(length(day.counts.ordered.top100$Name))

ggplot(day.counts.ordered.top100, aes(Rank,mean)) +
  geom_point(aes(order=Rank)) + geom_errorbar(day.limits) +
  scale_y_log10()+
...
library("tidyr")
library("dplyr")
night.counts=countData[, c("Night_4","Night_5","Night_6")]
night.counts$Name=rownames(night.counts)
night.counts.ordered = night.counts %>% 
  gather(Days, Counts, -Name) %>%
  group_by(Name) %>%
  summarise(Totals=sum(Counts), mean=mean(Counts), se=sd(Counts)/sqrt(length(Counts))) %>%
  arrange(desc(mean))
night.counts.ordered.top100=night.counts.ordered[1:500 ,]
night.limits=aes(ymax=mean+se, ymin=mean-se)
night.counts.ordered.top100$Rank = seq(1,length(night.counts.ordered.top100$Name))


ggplot(night.counts.ordered.top100, aes(Rank,mean)) +
  geom_point(aes(order=Rank)) + geom_errorbar(night.limits) +
  scale_y_log10() +
  ylab("log10 Average Counts")+
  xlab("Top 500 Genes")
```
...
### Day vs Night
```{r d_vs_n_rank}
day.counts.ordered.top100$Sample="Day"
night.counts.ordered.top100$Sample="Night"
combined.counts=bind_rows(day.counts.ordered.top100,night.counts.ordered.top100) %>%
  arrange(desc(mean))
combined.counts$Rank = seq(1,length(combined.counts$Name))
combined.limits=aes(ymax=mean+se, ymin=mean-se)

ggplot(combined.counts, aes(Rank,mean, color=Sample)) +
  geom_point(aes(order=Rank), position="jitter") + geom_errorbar(combined.limits) +
  scale_y_log10() +
  ylab("log10 Average Counts")+
...
```{r annotate_counts}
annotation=read.table("all_combined.products", sep="\t", quote="")
colnames(annotation)=c("Name", "IMG_Product","IMG_Source")
annotation$Name=as.character(annotation$Name)

combined.union=as.data.frame(union(day.counts.ordered.top100$Name,night.counts.ordered.top100$Name))
...
combined.union=left_join(combined.union, day.counts.ordered.top100, by="Name")
combined.union=left_join(combined.union, night.counts.ordered.top100, by="Name")
...
combined.union=subset(combined.union, select=-c(Del1,Del2))
combined.anno=left_join(combined.union, annotation, by="Name")
```

```{r write_counts}
write.table(combined.anno, file="combined_raw_counts.tsv",sep="\t", quote=FALSE, row.names = FALSE);
```
...
Nothing! Infact, we get `r sum(res$padj < 0.79, na.rm=TRUE)` genes with the adjusted p-value less than or equal to `0.79` and then suddenly at `0.8` we get `r sum(res$padj < 0.8, na.rm=TRUE)`
...
resOrdered.top125=data.frame(resOrdered[1:125 ,])
resOrdered.top125=resOrdered.top125[c(7,1:6)]
resOrdered.top125=left_join(resOrdered.top125, tmp.countData, by="Name")
resOrdered.top125=left_join(resOrdered.top125, day.counts.ordered, by="Name")
resOrdered.top125=left_join(resOrdered.top125, night.counts.ordered, by="Name")
resOrdered.top125=left_join(resOrdered.top125, annotation, by="Name")
colnames(resOrdered.top125)[14:19]=c("Day_Total", "Day_Mean", "Day_SE", "Night_Total", "Night_Mean", "Night_SE")

write.table(as.data.frame(resOrdered.top125), sep="\t",file="results.tsv", quote = FALSE, row.names =FALSE)
```
