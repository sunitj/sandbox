...
### Plot
p=ggplot(na.omit(combined.counts), aes(RankByMean,Mean, color=Sample)) +
...
return(list(day=day.counts.ordered, night=night.counts.ordered, combined=combined.counts, plot=p))
}
```
...
rank_abundance_by_bin= function(bin, title){
	...
	head(bin.norm.countData)
	myObj=rank_abundance(bin.norm.countData,title)
	return(myObj)
}
```
...
# plots
for(x in 1:length(foreach.cyano.bin$Bin)){
	bin.rank.abundance[[x]]$plot
}
```

<!-- Can't use this block unless I figure out how to return multiple objects from a function.
