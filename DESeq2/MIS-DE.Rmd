...
bin.summary.num=scaff.bins %>%
	group_by(Bin) %>%
#	summarise(BinLen=sum(Length),MedianLen=median(Length), MeanLen=mean(Length), MinLen=min(Length), MaxLen=max(Length))
...
bin.norm.countData=inner_join(raw.counts,scaff.bins, by="Scaffolds") %>% 
	...
	replace(is.na(.),0) %>% 
	mutate(Total=sum(n.Day1,n.Day2,n.Day3,
									 n.Night4,n.Night4,n.Night6))
...
grouped_counts=function(df=df,label=""){
	gc.df= df %>% 
	  ...
	  summarise(Totals=sum(Counts), Mean=mean(Counts), SE=sd(Counts)/sqrt(length(Counts))) %>%
		ungroup() %>% 
		top_n(topN, Mean) %>%
		mutate(Rank=min_rank(desc(Mean)), Sample=label)
...
bin.countData=inner_join(raw.counts,scaff.bins, by="Scaffolds") %>% 
	filter(Bin %in% cyano.bins$Bin)
```
...
The following are Rank Abundance plots by Cyano bins, normalized by Coverage as described in the formula above.

```{r rank_abundance_by_binCov_all_cyanos}
foreach.cyano.bin=cyano.bins %>% 
	select(1:6) %>% 
	unite("title",2:6, sep=" > ")

bin.rank.abundance=apply(foreach.cyano.bin,1,function(x) rank_abundance_by_bin(x[1],paste(x[1],x[2], sep= " = ")))

# plots
for(row.num in 1:length(foreach.cyano.bin$Bin)){
...
get_annotations= function(day,night,count.data){
...
bin.anno=lapply(bin.rank.abundance, function(x) get_annotations(x$day,x$night,bin.norm.countData))
# write
for(row.num in 1:length(bin.anno)){
	filename=paste("data/",important.bins$Bin[row.num],"_combined_raw_counts.tsv")
	write.table(bin.anno[[row.num]], file=filename,sep="\t", quote=FALSE, row.names = FALSE, na = "0");
}
...
<!-- dependent on day and night count data.
