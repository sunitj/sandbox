...
all.bin.reads=inner_join(scaff.bins,rc.scaff, by="Scaffolds") %>% 
	group_by(Bin) %>% 
	summarise_each(funs(sum), matches("_"))

cyano.bin.reads=inner_join(cyanos, all.bin.reads, by="Bin") %>% 
	group_by(Bin) %>% 
...
countData=inner_join(raw.counts,scaff.bins, by="Scaffolds") %>% 
	inner_join(., all.bin.reads, by="Bin") %>% 
...
countData=select(countData, -Genes, -Total)
colnames(countData)=c("Day_1","Day_2", "Day_3", "Night_4", "Night_5", "Night_6")
colSums(countData)
```
```{r}
...
vplot(countData, "Violin Plot for Bin Normalized Counts")
```
...
ggplot(na.omit(combined.counts), aes(RankByMean,Mean, color=Sample)) +
